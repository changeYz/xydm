...
<div class="rt-bar">
    <div class="row">
		...
		<a href="<?php echo U('Mh/index');?>"class="header-home">
			<i class="my-home"></i>
		</a>
    </div>
</div>
...
<div class="user_info">
	<div class="headimg">
		<div class="avatar">
			<img width="65" height="65" id="showheadimg" src="<?php echo ($user["headimg"]); ?>"" />
		</div>
	</div>
	<div class="info">
		<div>
			<span style="font-size:0.4rem;">账号: <?php echo ($user["username"]); ?></span>&nbsp;&nbsp;
			<span style="font-size:0.3rem;color:#ee2112;">
				权益：
				<?php if($user['vip']): ?>VIP
				<?php else: ?>
					普通会员<?php endif; ?>
				/
				<?php echo (floatval($user["money"])); ?>书币
			</span>
		</div>
	    <div>
			<span style="color:#99b7cf">网址:<?php echo ($webUrl); ?></span>
		</div>
	</div>
</div>
<div class="userId">
   充值ID：<?php echo ($user["id"]); ?>, 登陆账号：<?php echo ($user["username"]); ?>, 密码: ******
</div>
<!--用户信息结束-->
	...
	<?php if(is_array($_charge)): foreach($_charge as $key=>$v): ?><div class="item">
			...
			<a href="javascript:void(0);" class="container" data-fee="<?php echo ($v['money']); ?>" _vip="0">
				<div class="body">
					...
					<div class="bio"><span style="color:#ff730b;"><?php echo ($v['money']*$_site['rate']); ?>+<?php echo ($v['send']); ?></span> <span style="font-size: .28rem;color:#cccccc">书币</span></div>
	...
	<?php if($_site['vip']): ?><div class="item">
		...
		<a href="javascript:void(0);" class="container" data-fee="<?php echo ($_site['vip']); ?>" _vip="1">
			<div class="body">
				...
				<div class="bio"><span style="font-size: .28rem;color:#cccccc">VIP包年</span></div>
