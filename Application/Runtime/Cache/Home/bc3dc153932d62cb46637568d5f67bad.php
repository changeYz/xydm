...
<div class="rt-bar" style="position: fixed">
    <div class="row">
        <div class="col icon">
            <a href="<?php echo U('Book/bookinfo');?>&bid=<?php echo ($bid); ?>">
...
<div class="read-footer">
    ...
    <div class="row">
        ...
        <div class="col">
            <a href="<?php echo U('Book/bookinfo');?>&bid=<?php echo ($info["bid"]); ?>&isji=1" style="-webkit-tap-highlight-color: rgba(0,0,0,0);">
 ...
 $before = $info['before']; if($before <= 1) { $before = 1; } $next = $info['next']; $money = M('book_episodes')->where(array('ji_no'=>$next,'bid'=>$binfo['id']))->getField('money'); $money = $money?$money:$_site['jimoney']; $read = M('book_read')->where(array('ji_no'=>$next,'bid'=>$binfo['id'],'user_id'=>$user['id']))->find(); ?>
            <a href="javascript:;" onclick="before(<?php echo ($binfo['id']); ?>,<?php echo ($before); ?>)"><i></i>上一章</a>
        </div>
        <div class="item next">
			<?php if($next >= $binfo['pay_num'] and $binfo['pay_num'] > 0): if($read): ?><a href="javascript:;" onclick="next(<?php echo ($binfo['id']); ?>,<?php echo ($next); ?>,<?php echo ($binfo['pay_num']); ?>)">下一章<i></i></a>
				<?php else: ?>
					<a href="javascript:;" onclick="next(<?php echo ($binfo['id']); ?>,<?php echo ($next); ?>,<?php echo ($binfo['pay_num']); ?>)">下一章<i></i></a><?php endif; ?>
			<?php else: ?>
				<a href="javascript:;" onclick="next(<?php echo ($binfo['id']); ?>,<?php echo ($next); ?>,<?php echo ($binfo['pay_num']); ?>)">下一章<i></i></a><?php endif; ?>
        </div>
    </div>
...
<script type="text/javascript">
	...
	function before(bid,before){
		location.href="<?php echo U('Book/inforedit');?>&bid="+bid+"&ji_no="+before+"&isNight="+isNight+"&fsize="+fsize+"&ps="+ps;
	}
